...
def test_stride_dilation():
    ...
    Y = topi.nn.conv2d(X, W, 2, 0, 2)
    check_grad(Y, [X, W], perf=(1057, 1968, 467))
    Y = topi.nn.conv2d(X, W, 3, 0, 2)
    check_grad(Y, [X, W], perf=(561, 6064, 425))
    Y = topi.nn.conv2d(X, W, 1, 0, 3)
    check_grad(Y, [X, W], perf=(2585, 10352, 417))
    Y = topi.nn.conv2d(X, W, 2, 0, 3)
    check_grad(Y, [X, W], perf=(673, 4160, 425))
    Y = topi.nn.conv2d(X, W, 3, 0, 3)
    check_grad(Y, [X, W], perf=(801, 1488, 449))
    ...
    Y = topi.nn.conv2d(X, W, 3, 0, 2)
    check_grad(Y, [X, W], perf=(689, 8416, 509))
    Y = topi.nn.conv2d(X, W, 1, 0, 3)
    ...
    check_grad(Y, [X], perf=(785, 5512, 529))
